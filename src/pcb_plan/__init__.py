...
``src/pcb-plan/pcb_plan.py``. This package exists so installed console scripts
...
_IMPL_PATH = Path(__file__).resolve().parents[1] / "pcb-plan" / "pcb_plan.py"
_SPEC = importlib.util.spec_from_file_location("_pcb_plan_impl", _IMPL_PATH)
if _SPEC is None or _SPEC.loader is None:  # pragma: no cover - defensive import guard
    raise ImportError(f"Unable to load pcb-plan implementation from {_IMPL_PATH}")

...
`pcb-place` is the deterministic footprint placement executor for KiCad `.kicad_pcb` files.
...
`pcb-place` is designed to complement hardware-as-code flows such as [`pcb`](https://github.com/diodeinc/pcb) and Zener, and to serve as the deterministic execution stage in a broader board-as-code build definition workflow:
...
`pcb-place` remains a deterministic placement executor: it applies the `.ppl` part-placement language to a KiCad board and does not infer intent, route traces, or run a global optimizer. `pcb-plan` can read richer design artifacts and `.pln` board-plan files, then emit `.ppl` rules that target these deterministic primitives while keeping heuristic planning outside this executor.
